...
void glxy::RulerUI::addLabel(const bool isVertical, const Vector2f position, const float scale)
{
    labels.emplace_back(font);
    labels.back().setCharacterSize(16 * config.GUIScale);
    if (!isVertical)
    {
        labels.back().setPosition(position + Vector2f(scale * 3.f, 0));
        labels.back().setString(to_string(static_cast<int32_t>(std::round(position.x))));
        labels.back().setOrigin(Vector2f());
    }
    else
    {
        labels.back().setPosition(position + Vector2f(0, scale * 3.f));
        labels.back().setString(to_string(static_cast<int32_t>(std::round(position.y))));
...
glxy::RulerUI::RulerUI(const View& view, const Font& font)
    : view(view), font(font)
{

}

void glxy::RulerUI::setEnabled(const bool enabled)
{
    this->enabled = enabled;
}

void glxy::RulerUI::setThemeColor(const Color color)
{
    this->themeColor = color;
}

void glxy::RulerUI::Start()
{
...
bool glxy::RulerUI::Update(const Vector2f viewportSize, const Vector2f mousePos)
{
    if (!enabled)
        return false;
    bool updated = false;
    ...
    selectionX.at(0) = {Vector2f(std::floor(mousePos.x),
    view.getCenter().y - view.getSize().y / 2.f), themeColor};
    selectionX.at(1) = {Vector2f(std::floor(mousePos.x),
        view.getCenter().y - view.getSize().y / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale), themeColor};
    selectionX.at(2) = {Vector2f(std::ceil(mousePos.x),
        view.getCenter().y - view.getSize().y / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale), themeColor};
    selectionX.at(3) = {Vector2f(std::ceil(mousePos.x),
        view.getCenter().y - view.getSize().y / 2.f), themeColor};

    selectionY.at(0) = {Vector2f(view.getCenter().x - view.getSize().x / 2.f,
        std::floor(mousePos.y)), themeColor};
    selectionY.at(1) = {Vector2f(view.getCenter().x - view.getSize().x / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale,
        std::floor(mousePos.y)), themeColor};
    selectionY.at(2) = {Vector2f(view.getCenter().x - view.getSize().x / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale,
        std::ceil(mousePos.y)), themeColor};
    ...
    if (view.getCenter() != lastView.getCenter() || view.getSize() != lastView.getSize() || manualChange)
    {
        ...
        blackBg.at(0) = {
            Vector2f(view.getCenter().x - view.getSize().x / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale,
            view.getCenter().y - view.getSize().y / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale), Color::Black};
        blackBg.at(1) = {
        ...
        blackBg.at(4) = {Vector2f(view.getCenter().x - view.getSize().x / 2.f,
            view.getCenter().y + view.getSize().y / 2.f), Color::Black};
        blackBg.at(5) = {Vector2f(view.getCenter().x - view.getSize().x / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale,
            view.getCenter().y + view.getSize().y / 2.f), Color::Black};

        vertexArray.append({Vector2f(view.getCenter().x - view.getSize().x / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale,
            view.getCenter().y - view.getSize().y / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale)});
        ...
        //small on x axis
        if (spacingSmall > 0)
            for (int32_t x = std::ceil((view.getCenter().x - view.getSize().x / 2 + pixelsPerPixel * c_rulerSize * config.GUIScale) / spacingSmall) * spacingSmall;
                x < view.getCenter().x + view.getSize().x / 2; x += spacingSmall)
            {
                ...
                vertexArray.append({Vector2f(x, view.getCenter().y - view.getSize().y / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale)});
            }
        //large on x axis
        for (int32_t x = std::ceil((view.getCenter().x - view.getSize().x / 2 + pixelsPerPixel * c_rulerSize * config.GUIScale) / spacing) * spacing;
        ...
        //small on y axis
        if (spacingSmall > 0)
            for (int32_t y = std::ceil((view.getCenter().y - view.getSize().y / 2 + pixelsPerPixel * c_rulerSize * config.GUIScale) / spacingSmall) * spacingSmall;
                y < view.getCenter().y + view.getSize().y / 2; y += spacingSmall)
            {
                ...
                vertexArray.append({Vector2f(view.getCenter().x - view.getSize().x / 2.f + pixelsPerPixel * c_rulerSize * config.GUIScale, y)});
            }
        //large on y axis
        for (int32_t y = std::ceil((view.getCenter().y - view.getSize().y / 2 + pixelsPerPixel * c_rulerSize * config.GUIScale) / spacing) * spacing;
...
void glxy::RulerUI::draw(RenderTarget& target, RenderStates states) const
{
    if (!enabled)
        return;
    target.setView(view);
