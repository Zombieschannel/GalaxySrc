...
ImVec4 ColorPicker::getColor(const ColorID colorID) const
{
...
void ColorPicker::setColor(const ColorID colorID, const ImVec4& color)
{
    colors.at(colorID) = color;
    colorChanged = true;
}

void ColorPicker::setEditorColors(const ColorID colorID, const ImVec4& color)
{
    colors.at(colorID) = color;
}
        ...
        ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
    {
        ...
        if (ImGui::BeginChild("Scroll", Vector2f(0, -75 * GUIScale), ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            if (c_colorCount == 2)
            {
                if (editingColor == 0)
                {
                    if (ImGui::ColorPicker4(("Primary"_S + "##Color").c_str(), &colors.at(0).x, ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreview |
                        (colorPickerTriangle ? ImGuiColorEditFlags_PickerHueWheel : ImGuiColorEditFlags_PickerHueBar), &oldColors.at(0).x))
                        colorChanged = true;
                }
                else if (editingColor == 1)
                {
                    if (ImGui::ColorPicker4(("Secondary"_S + "##Color").c_str(), &colors.at(1).x, ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreview |
                        (colorPickerTriangle ? ImGuiColorEditFlags_PickerHueWheel : ImGuiColorEditFlags_PickerHueBar), &oldColors.at(1).x))
                        colorChanged = true;
                }
            }
        }
