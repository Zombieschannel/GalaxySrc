...
void SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height)
{
    ...
    if (shaderHandle != shader.getNativeHandle())
    {
        shaderHandle = shader.getNativeHandle();
        posAttrib = glGetAttribLocation(shader.getNativeHandle(), "position");
        colAttrib = glGetAttribLocation(shader.getNativeHandle(), "color");
        texAttrib = glGetAttribLocation(shader.getNativeHandle(), "texCoord");
    }
    if (posAttrib >= 0)
    ...
    float tx = -(right + left) / (right - left), ty = -(top + bottom) / (top - bottom), tz = -(_far + _near) / (_far - _near);

    float mat[16] = { 2.f / (right - left), 0, 0, 0,
                      0, 2.f / (top - bottom), 0, 0,
                      0, 0, -2.f / (_far - _near), 0,
                      tx, ty, tz, 1
    };

    const_cast<sf::Shader*>(&sf::Shader::getDefaultTexShader())->setUniform("sf_projection", static_cast<sf::Glsl::Mat4>(mat));
